...
class CycleSYNTHIA(data.Dataset):
	...
	def label_path(self, filename):
		...
		# danny: return os.path.join(self.root, 'GT', 'LABELS', filename)
	...
	def __getitem__(self, index, debug=False):
		...
		#danny: img = Image.open(img_path)
		if self.transform is not None:
			img = self.transform(img)
		target = Image.open(label_path)
		#danny target = Image.open(label_path).convert('L')
